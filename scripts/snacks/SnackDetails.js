export const SnackDetails = (snackObject, snackToppingObj) => {
  let toppingStr = "";
  snackToppingObj.forEach((thisTopping, index) => {
    if (snackToppingObj.length === 1) {
      toppingStr = thisTopping.topping.name;
    } else {
      if (index === snackToppingObj.length - 1) {
        toppingStr += ` and ${thisTopping.topping.name}`;
      } else {
        toppingStr += `${thisTopping.topping.name}, `;
      }
    }
				...
				<h5 color="primary">${snackObject.name}</h5>
				  ...
				  <div class="container">
					...
					<div class="row row-cols-1">
						<div class="col col-details">${toppingStr}</div>
					</div>
				</div>
